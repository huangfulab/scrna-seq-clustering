# conda run -n <conda.env_name> Rscript step10_cell_type/scripts/plot.R
source(here::here("init.R"))

out_dir <- here("step10_cell_type", "figs")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

# markers / cell_type_map / ct_levels come from init.R

# NOTE: renumbering vs. the original pipeline's step11_cell_type — that step
# had 6 figs. fig5 (guide_origin composition) is guide-specific and dropped
# entirely — no guide capture in the plain profile — so fig6 (phase
# composition) shifts down to fig5. fig3's qc_vars also drops MOI.

# =============================================================================
# LOAD
# =============================================================================
cat("Loading obj9_cell_type...\n")
obj <- qs_read(here("step10_cell_type", "obj9_cell_type.qs2"), nthreads = n_cores)
...
# =============================================================================
# fig3: Violin QC — 4 metrics (MOI dropped, no guide capture), grouped by CellType
# =============================================================================
cat("[fig3] Violin QC...\n")
...
# =============================================================================
# fig5 (was fig6): Phase composition
# =============================================================================
cat("[fig5] Phase composition...\n")
p <- composition_bar(obj, "Phase", phase_cols, "Phase")
ggsave(file.path(out_dir, "fig5_phase_composition.png"), p,
       width = bar_width, height = 5, dpi = 300, bg = "white")
...
cat("\n---\n")
cat("STOP: this is the end of the pipeline — no more steps to write.\n")
cat("Open fig1_umap_celltype.png for the final annotated UMAP, fig2_dotplot_markers.png to\n")
cat("re-confirm each cell type's marker signature, and fig3-5 for QC/lane/phase composition per\n")
cat("cell type (same sanity checks as before: no cell type should be driven by one lane or show\n")
cat("QC values wildly different from the rest). obj9_cell_type.qs2 in this step's directory is the\n")
cat("final annotated object for downstream analysis. No new config value is needed — just confirm\n")
cat("everything looks right.\n")
