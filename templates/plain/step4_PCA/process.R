...
# Load ----------
stopifnot(file.exists(here("step3_filter", "obj3_filter.qs2")))
cat("Loading obj3_filter...\n")
obj <- qs_read(here("step3_filter", "obj3_filter.qs2"), nthreads = n_cores)
...
# ScaleData ----------
cat("\n[4] ScaleData (vars.to.regress:", paste(unlist(config$pca$vars_to_regress), collapse = ", "), ")...\n")
plan("multicore", workers = n_cores)
obj <- ScaleData(obj,
                 features        = VariableFeatures(obj),
                 vars.to.regress = unlist(config$pca$vars_to_regress),
                 verbose         = TRUE)
plan("sequential")
...
# Save ----------
cat("\nSaving obj4_PCA...\n")
obj4_path <- here("step4_PCA", "obj4_PCA.qs2")
qs_save(obj, file = obj4_path, nthreads = n_cores)
cat(glue("Saved: step4_PCA/obj4_PCA.qs2 ({round(file.size(obj4_path)/1e6, 1)} MB)\n"))

# Verification ----------
cat("\n=== Verification ===\n")
cat("Reductions:", paste(Reductions(obj), collapse = ", "), "\n")
cat("Layers:", paste(Layers(obj), collapse = ", "), "\n")
cat("\n=== STEP 4 COMPLETE — run step4_PCA/plot.R, then inspect elbow plot ===\n")
