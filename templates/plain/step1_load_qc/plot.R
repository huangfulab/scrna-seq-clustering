# conda run -n <conda.env_name> Rscript step1_load_qc/scripts/plot.R
source(here::here("init.R"))

figs_dir <- here("step1_load_qc", "figs")
tables_dir <- here("step1_load_qc", "tables")
...
qc_meta <- map(lane_names, function(l) {
  obj <- qs_read(here("step1_load_qc", "obj0_load_qc", glue("{l}.qs2")), nthreads = n_cores)
  cat(" ", l, "\n")
...
ggsave(file.path(figs_dir, "fig1_qc_violin.png"), p, width = 14, height = 8, dpi = 300, bg = "white")
...
cat("fig1 will regenerate with the confirmed lines — before step2 is written.\n")
